...
/* Code prefixes generated by the system — see lib/firebase/firestore.ts
   for the canonical generators. Each prefix maps to a redemption flow:
     JOIN-      → team activation (member joining)
     SHARETEAM- → team clone (leader cloning another team)
     ACTIVATE-  → training activation (learner unlocking)
     SHARE-     → training clone (leader cloning a training)
     PIPE-      → pipeline clone
     FUNNEL-    → shared-funnel clone */
function detectKind(code: string): CodeKind {
  const c = code.trim().toUpperCase();
  if (c.startsWith("JOIN-") || c.startsWith("SHARETEAM-")) return "team";
  if (c.startsWith("ACTIVATE-") || c.startsWith("SHARE-")) return "training";
...
export function InviteCodeCard() {
  ...
  const submit = (e?: React.FormEvent) => {
    ...
    if (!target) {
      toast.error(
        "That code doesn't look right. Team codes start with JOIN-, trainings with ACTIVATE-.",
      );
      return;
    }
    router.push(target);
